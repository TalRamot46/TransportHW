...
\subsection*{The two regions, and the solutions to Helmholtz on each}
...
\emph{Forward sweep}, $\mu_m>0$: the flux enters at $x_{i-1/2}$ and leaves at
...
\begin{equation}
    ...
    = \frac{S_i}{2}\Delta x
\end{equation}
and collecting $\psi_{i,m}$ gives the cell centre and then, from the closure
...
\emph{Backward sweep}, $\mu_m<0$: the roles swap. Writing $\mu_m=-|\mu_m|$ turns
...
\end{equation}
marched from the vacuum face at $x=a/2$ inwards, starting from
...
\paragraph{The two faces of a spherical cell.} Equation \eqref{eq:sphbalance} is
\eqref{eq:slabbalance} with a second outgoing face. They are not alike and it is worth naming
them separately:
\begin{itemize}
    \item the \emph{radial face}, weighted by the two shell areas
    ...
    slab.
    \item the \emph{angular face}, weighted by the two coefficients
...
Take the forward sweep, $\mu_m>0$, so that the radial outflow is
...
\begin{multline}
    ...
    \label{eq:sphcollect}
\end{multline}
Every term now carries either $\psi_{i,m}$ or a known inflow, so collecting
...
\begin{equation}
    ...
    \label{eq:cellflux}
\end{equation}
and \eqref{eq:closr} and \eqref{eq:closmu} then hand on the two fluxes the
...
$A_{\text{in}}=A_{i+1/2}$, $\mu_m$ replaced by $|\mu_m|$ and the radial inflow (eq. \eqref{eq:closr}) taken as
