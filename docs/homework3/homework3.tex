...
A homogeneous slab of total thickness $2a$, reflective at the midplane $x=0$ and vacuum at
$x=a$, in mean free paths ($\Sigma_t=1$), for $c\in\{1.2,1.5,1.8\}$ and $N\in\{1,3,5,9\}$.
The unknown is the half-thickness $a_{\text{crit}}$ at which $k_{\text{eff}}=1$.

\paragraph{A notational warning.} This section's $a$ is the \emph{half}-thickness, so it is
what Question 3 calls $a/2$. The two tables are comparable entry by entry only after that
identification.

\subsection*{The $P_N$ equations}

With isotropic scattering and fission lumped as in the paragraph ``The meaning of $c$'' before Question 3,
and the fission source divided by $k$,
\begin{equation}
...
At $x=a$ no neutron enters, $\psi(a,\mu)=0$ for $\mu<0$, which $N+1$ moments cannot satisfy
pointwise. Marshak's choice is to annihilate the $\tfrac{N+1}{2}$ lowest odd moments of the
incoming half-range,
\begin{equation}
    \int_{-1}^{0}\mu^{2m-1}\psi(a,\mu)\,d\mu = 0
    \;\Longrightarrow\;
    \sum_{n=0}^{N} c_{mn}\,\phi_n(a) = 0,
    \qquad
...
For $N=1$, \eqref{eq:marshak} reads $-\tfrac14\phi_0(a)+\tfrac12\phi_1(a)=0$, i.e.\
$\phi_0(a)=2\phi_1(a)$: the classical statement that the scalar flux at a vacuum surface is
...
\paragraph{The scheme.} Divide $[0,a]$ into $J$ cells of width $\Delta x = a/J$ with nodes
...
productions,
\begin{equation}
    k^{(\ell+1)} = k^{(\ell)}\,\frac{\displaystyle\int_0^a \phi_0^{(\ell+1)}\,dx}
                                    {\displaystyle\int_0^a \phi_0^{(\ell)}\,dx}
    \label{eq:kupdate}
\end{equation}
...
\begin{equation}
    \phi_0^{(0)}(x)=\cos\!\left(\frac{\pi x}{2\,(a+0.7104)}\right),\qquad k^{(0)}=1
\end{equation}
...
\begin{equation}
    f(a) = k(a) - 1
\end{equation}
exactly as in Questions 3--5. $k(a)$ is monotone increasing --- a thicker slab leaks a smaller
fraction --- and runs from $k\to0$ as $a\to0$ to the infinite-medium value $k\to c$ as
$a\to\infty$. So for every $c>1$ a root exists and is unique, and any bracket
$[a_{\text{lo}},a_{\text{hi}}]$ straddling the diffusion estimate below is admissible;
...
$a_{\text{crit}}$ falls as $\Delta x^2$ and is separable from the truncation error in $N$ by
...
$\boldsymbol{\Phi}_{\text{e}}=(\phi_0,\phi_2,\dots)$ and
$\boldsymbol{\Phi}_{\text{o}}=(\phi_1,\phi_3,\dots)$. Every derivative in \eqref{eq:pnrec}
changes the parity of its index, so the even-$n$ and odd-$n$ equations separate:
\begin{equation}
    \mathbf{A}\frac{d\boldsymbol{\Phi}_{\text{o}}}{dx} + \mathbf{\Sigma}_0(k)\,\boldsymbol{\Phi}_{\text{e}} = 0,
    \qquad
    \mathbf{B}\frac{d\boldsymbol{\Phi}_{\text{e}}}{dx} + \boldsymbol{\Phi}_{\text{o}} = 0,
    \qquad
...
$\boldsymbol{\Phi}_{\text{o}}=-\mathbf{B}\,\boldsymbol{\Phi}_{\text{e}}'$ into the first
equation eliminates the odd moments and leaves a coupled Helmholtz system,
\begin{equation}
    \frac{d^2\boldsymbol{\Phi}_{\text{e}}}{dx^2} + \mathbf{K}^2(k)\,\boldsymbol{\Phi}_{\text{e}} = 0,
    \qquad
...
$\mathbf{A}\mathbf{B}\,\boldsymbol{\Phi}_{\text{e}}''=\mathbf{\Sigma}_0\boldsymbol{\Phi}_{\text{e}}$,
...
$\boldsymbol{\Phi}_{\text{o}}=-\mathbf{B}\boldsymbol{\Phi}_{\text{e}}'$ vanishes wherever
$\boldsymbol{\Phi}_{\text{e}}'$ does, is
\begin{align}
    \boldsymbol{\Phi}_{\text{e}}(x) &= C_1\mathbf{v}_1\cos(B_0x)
        + \sum_{j=2}^{M} C_j\mathbf{v}_j\cosh(\kappa_jx) \\
    \boldsymbol{\Phi}_{\text{o}}(x) &= C_1(\mathbf{B}\mathbf{v}_1)B_0\sin(B_0x)
...
$\mathbf{M}_{\text{e}},\mathbf{M}_{\text{o}}$ and evaluating at $x=a$ gives $M$ homogeneous
equations $\mathbf{H}(a)\mathbf{C}=\mathbf{0}$ with
\begin{equation}
    \mathbf{H}_{\cdot1}=\mathbf{M}_{\text{e}}\mathbf{v}_1\cos(B_0a)
        +\mathbf{M}_{\text{o}}(\mathbf{B}\mathbf{v}_1)B_0\sin(B_0a),
    \quad
    \mathbf{H}_{\cdot j}=\mathbf{M}_{\text{e}}\mathbf{v}_j\cosh(\kappa_ja)
        -\mathbf{M}_{\text{o}}(\mathbf{B}\mathbf{v}_j)\kappa_j\sinh(\kappa_ja)
\end{equation}
and a non-trivial flux exists only where
\begin{equation}
    \boxed{\;\det\mathbf{H}(a)=0\;}
    \label{eq:hdet}
\end{equation}
$a_{\text{crit}}$ is its smallest positive root. Two numerical points: the $\cosh$ and $\sinh$
columns overflow once $\kappa_ja\gtrsim700$, so each is divided by its own $\cosh(\kappa_ja)$,
which leaves $\mathbf{v}_j$ and $-(\mathbf{B}\mathbf{v}_j)\kappa_j\tanh(\kappa_ja)$ and cannot
...
$B=\sqrt{3(c-1)}$ and $\phi_0=\cos(Bx)$, the Marshak condition $\phi_0(a)=2\phi_1(a)$ and
$\phi_1=-\tfrac13\phi_0'$ give $\cot(Ba)=\tfrac23B$, that is
\begin{equation}
    a_{\text{crit}}^{P_1} = \frac{1}{B}\arctan\!\left(\frac{3}{2B}\right)
\end{equation}
...
\bottomrule
\end{tabular}
\caption{Analytic $P_1$ half-thickness in mean free paths, against Question 3's $S_2$ column (there
tabulated as $a/2$). The Mark column, $\arctan(\sqrt3/B)/B$, is the same equation with the
...
\bottomrule
\end{tabular}
\caption{Critical half-thickness $a_{\text{crit}}$ in mean free paths. Method 1 is the box
scheme on 200 cells; Method 2 is $\det\mathbf{H}(a)=0$. The gap between them is the relative
...
\caption{Left: convergence of $a_{\text{crit}}$ with $N$, dashed lines the exact-transport
...
\begin{equation*}
    0.723479\;(P_1),\quad 0.629148\;(P_3),\quad 0.612135\;(P_5),\quad 0.606466\;(P_9),\quad
    0.605952\;(P_{11}),\quad 0.605550\;(P_{15}),\quad 0.605181\;(P_{31})
\end{equation*}
