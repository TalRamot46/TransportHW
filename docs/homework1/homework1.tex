...
\paragraph{Neutron balance.} For the converged critical flux, production must equal absorption plus leakage. The informative term is the leakage: production and absorption are the same volume integral times a constant, so their ratio $\nu\Sigma_f/(k\Sigma_a)$ holds identically whatever the flux. Leakage, by contrast, is computed from the boundary formula using only $\phi_{N-1}$, and nothing forces it to close the balance unless the discrete operator conserves neutrons cell by cell and the boundary coefficient is exactly right.

\begin{table}[htbp]
    \centering
    \caption{Neutron balance at the critical radius, classical diffusion on a 400-cell mesh, for the test media $\Sigma_t = \Sigma_a = 1$, $\nu\Sigma_f = c$. The flux is normalised to unit maximum, so the absolute values are arbitrary but their ratios are not. The asymptotic branch gives smaller spheres and hence smaller terms, but identical fractions and residuals of the same order.}
    \label{tab:q4_balance}
    \begin{tabular}{lrrrccc}
        \toprule
        $c$ & $P$ & $A$ & $L$ & $L/P$ & $(c-1)/c$ & $|P - A - L|/P$ \\
        \midrule
        1.02 & 2739.887787 & 2686.164497 & 53.723290 & 0.019608 & 0.019608 & $5.9\times10^{-15}$ \\
        1.10 & \phantom{0}264.283575 & \phantom{0}240.257796 & 24.025780 & 0.090909 & 0.090909 & $9.6\times10^{-15}$ \\
        1.20 & \phantom{0}101.932749 & \phantom{00}84.943958 & 16.988792 & 0.166667 & 0.166667 & $1.9\times10^{-14}$ \\
        1.50 & \phantom{00}32.233965 & \phantom{00}21.489310 & 10.744655 & 0.333333 & 0.333333 & $2.3\times10^{-14}$ \\
        1.80 & \phantom{00}19.112390 & \phantom{00}10.617995 & \phantom{0}8.494396 & 0.444444 & 0.444444 & $1.1\times10^{-13}$ \\
        2.00 & \phantom{00}15.195237 & \phantom{000}7.597619 & \phantom{0}7.597619 & 0.500000 & 0.500000 & $5.1\times10^{-14}$ \\
        \bottomrule
    \end{tabular}
\end{table}

The residual sits at machine precision throughout --- between $6\times10^{-16}$ and $1.3\times10^{-13}$ over the whole sweep, for both approximations --- with no trend in $c$, which is why it is tabulated rather than plotted. The two columns that do carry information are the last but one and its prediction. At criticality $k = 1$, so $A/P = \Sigma_a/(\nu\Sigma_f)$ and the balance forces
\begin{equation}
    \frac{L}{P} = 1 - \frac{\Sigma_a}{\nu\Sigma_f} = \frac{c-1}{c}
    \qquad \text{(the last form for these test media, where } \Sigma_s = 0\text{)}
    \label{eq:leakfrac}
\end{equation}
and the measured fractions reproduce it to every digit shown, identically for both approximations despite their different radii. Only the first equality is a tautology; the second requires the balance to close, so this is the same check as the residual column, read as physics rather than as round-off. Half the neutrons leak out of the critical sphere at $c = 2$, and a third at $c = 1.5$ --- the quantitative reason the surface condition matters as much as it does.
...
These are one-group, isotropic-scattering results, not predictions of real critical masses: at about two mean free paths the boundary layer is a large fraction of the system. By \eqref{eq:leakfrac} with each material's own $\Sigma_a/\nu\Sigma_f$, $61.7\,\%$ of the neutrons produced in the critical Pu-239 sphere leak out of it, and $55.6\,\%$ in U-235 --- both measured to six digits and identical under either approximation. Diffusion of either flavour is therefore well outside its regime, and the transport benchmark radii of Sood \emph{et al.} are the appropriate reference. The remaining three benchmark rows (H$_2$O, Fe, Na) have $c \leq 1$ and are non-multiplying, so no bare critical sphere exists for them at any radius.
...
\section{The Spherical Finite-Volume Scheme in Detail}
\label{app:fv}

An expansion of \S\ref{sec:q4_discretisation}, following \texttt{src/homework1/spherical.py} line by line: what the mesh looks like, how \texttt{\_banded\_operator} turns it into a matrix, which index means what, and where the boundary conditions and the starting iterate enter.
...
\end{equation}
that is: applied to a flat flux, the operator returns absorption plus surface leakage and nothing else. This is the discrete identity behind the balance check of \S\ref{sec:q4_verification}; it holds by construction rather than by luck, which is why that check probes the boundary coefficient \eqref{eq:CN} and little else.
