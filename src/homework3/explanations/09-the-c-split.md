...
`k(l) - 1` independent of the split?**

## The operator that does not know about the split

Fix a half-thickness `l` and let `K_l` be the map

    K_l phi  =  integrate over mu of ( T_l^-1 (phi/2) )

— emit isotropically, transport once, collect the scalar flux — where `T_l` is streaming plus
...
never firing in Questions 2–5, see [02](02-sn-solver.md)). Hence
    ...
    Sigma_s + nu Sigma_f / k = 1 / lam0(l)

    k(l) = nu Sigma_f lam0(l) / ( 1 - Sigma_s lam0(l) )         <- split-dependent

    k = 1  <=>  Sigma_s + nu Sigma_f = 1 / lam0(l)  <=>  lam0(l) = 1 / (c Sigma_t)

The split survives in `k(l)` and cancels at `k = 1`, because there it enters only through the
sum, and the sum *is* `c Sigma_t`. The critical half-thickness is the root of an equation
`Sigma_s` does not appear in. Since `lam0` rises monotonically from 0 to `1/Sigma_t`, that root
exists and is unique exactly when `c > 1`.

Nothing in this used a continuum limit — only that `K_l` is linear and that both terms go
through it. So it holds for the 200-cell discrete operator too: the *computed* half-thickness,
discretisation error and all, is split-independent, which is what the report's tables need.

## Measured

`c = 1.5`, `S_8`, 200 cells. The critical half-thickness is constant to `1.1e-11` — `brentq`'s
own tolerance — while `k` at a *non-critical* half-thickness moves by a factor of two:

| `Sigma_s` | `nu Sigma_f` | critical `a/2` | `k` at `a/2 = 0.5` |
|---|---|---|---|
...
Stronger: taking the single value `lam0(0.5) = 0.614677468` from the `Sigma_s = 0` row and
predicting the other three from the formula above reproduces the measured `k` to `1e-9`. So
`K_l` really is the same operator in all four runs, and the split really does enter only
through that scalar bracket.

## What would break it

- **A source term that bypasses `K_l`.** The argument needs both terms to go through one
