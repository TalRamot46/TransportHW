...
\begin{align}
    \frac{\partial \phi}{\partial t} &\;\longrightarrow\; s\,\tilde{\phi} - \hat{\phi}(k,0^-) = s\,\tilde{\phi} \\W
...
Part 3(c) asks for \eqref{eq:vform} solved numerically for $t>0$ on the whole line. Before
discretising, the absorption is removed exactly, as in Step 2: with
...
\begin{equation}
    ...
    \label{eq:heat}
\end{equation}
This is exact, not an approximation, and it is worth doing: $c$ then enters only through $D$
and through the prefactor reapplied on the way out, so one solver covers both approximations
and every $c$, including $c>1$ where the prefactor grows instead of decaying.

All three solutions are even in $x$, so \eqref{eq:heat} is solved on the half domain $[0,L]$
with the symmetry condition $\partial u/\partial x = 0$ at $x=0$ and the truncation $u(L,t)=0$.
The Gaussian has variance $\sigma^2 = 2Dvt$, so taking
\begin{equation}
...
\paragraph{The grid, and the discrete pulse.} Take nodes $x_j = jh$, $j = 0\ldots N$ with
$h = L/N$, and levels $t^n = n\Delta t$. On a node-centred grid the trapezoid weight of node
...
\paragraph{Stability.} Substituting the Fourier mode $u_j^n = \xi^n e^{ij\theta}$ into
...
\end{equation}
This is a hard limit rather than an accuracy guideline: past it the $\theta = \pi$ mode grows
like $|1-4r|^n$ and the solution does not degrade, it explodes. The local truncation error is
$\mathcal{O}(\Delta t) + \mathcal{O}(h^2)$, so holding $r$ fixed makes the scheme
$\mathcal{O}(h^2)$ overall --- at the price that halving $h$ quarters $\Delta t$, and with
work per step proportional to $N$ the total cost grows as $h^{-3}$.
...
\end{equation}
Summing \eqref{eq:ftcs} against these weights, the second differences telescope to their two
end values and the $j=0$ row cancels the $j=1$ end exactly, leaving
\begin{equation}
...
\end{itemize}
The error profile of Figure~\ref{fig:q3c_error} falls as $1/t$, the signature of a fixed
offset in the variance of the computed Gaussian; since the third result rules out the source as
that offset, what remains is the scheme's own truncation.
...
\begin{figure}[htbp]
    ...
    standard deviations. Left, across the domain at three times --- the downward spikes are
    sign changes, where the error passes through zero. Right, the peak error at $t = 4$ against
