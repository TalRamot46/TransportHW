...
| `pn_modal.py` | Question 2 Method 2: the modal elimination and `det H(a/2) = 0`. |
...
From the bottom up:
    ...
    sn.cell_flux                  one cell, one Face per outgoing face
      <- Solver.sweep_angle       one ordinate, marched across the mesh
      <- Solver.sweep_all_angles  all ordinates once -> scalar flux
      <- sn.run_sn_for_source     repeats that until the scattering source settles
...
**one `sweep_angle` is one direction**, **one `sweep_all_angles` is every direction once**, and
...
**`Solver` is a duck type, not a base class.** `sn.k_eigenvalue` and `sn.run_sn_for_source`
...
all to `sn.py`, and it is the seam to use for any further geometry.
