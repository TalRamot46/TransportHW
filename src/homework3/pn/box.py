...
def k_eigenvalue(half_thickness, medium, N, n_cells=N_CELLS, tol=1e-9, max_iter=2000):
    ...
    for outer in range(1, max_iter + 1):
        ...
        if converged:
            return core.KResult(k, system.nodes, phi, outer)
...
def critical_half_thickness(medium, N, guess, n_cells=N_CELLS):
    ...
    return core.critical_size(
        lambda a: k_eigenvalue(a, medium, N, n_cells).k, guess)
