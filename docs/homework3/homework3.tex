...
\FloatBarrier
\section{Critical slab half-thickness by $P_N$}
...
what \S3 calls $a/2$. The two tables are comparable entry by entry only after that
identification.

\subsection*{The $P_N$ equations}

With isotropic scattering and fission lumped as in the paragraph ``The meaning of $c$'' above,
...
of $S_{N+1}$. That is the algebraic content of the claim in \S3 that the two methods share
...
twice the net current, and the origin of the extrapolation length $2/3$ quoted in \S3.
...
\S3 is not available, because $\mathbf{\tilde A}$ mixes moments that stream in both directions,
...
running the same $N$ on a refined mesh --- the check reported in \S3 for $S_N$, and the
...
the classic diffusion buckling $B=\sqrt{3(c-1)}$ of the table in \S1 --- with the sign dropped
...
\toprule
$c$ & $B=\sqrt{3(c-1)}$ & $P_1$ Marshak & $P_1$ Mark & $S_2$ of \S3 & exact transport \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Analytic $P_1$ half-thickness in mean free paths, against \S3's $S_2$ column (there
...
digits --- the demonstration promised in \S3 that $P_N$ and $S_{N+1}$ differ in slab geometry
