...
Parts 3(a)--3(c) are covered: the exact-transport relations, and classical diffusion with the Marshak and Mark conditions. \textcolor{blue}{Parts 3(d)--3(e) follow at the end of the section.}
...
\begin{notebox}[The sign of the quadratic correction in \eqref{eq:z0fit}][blue]
...
\begin{notebox}[How good is the two-term fit for $z_0$?][blue]
...
\subsection*{\textcolor{blue}{Parts 3(d) and 3(e): Asymptotic Diffusion}}
{\color{blue}
Every part of this question uses the same relations \eqref{eq:q3relations}, so a part is fixed
by naming \emph{two} inputs, not one: the relaxation length $\ell$ inside $\cos(x/\ell)$, and
the boundary closure. Parts (b) and (c) take $\ell = 1/\sqrt{3(c-1)}$; parts (a), (d) and (e)
take $\ell = |\nu_0|$. Once the theory fixes $\ell$, the closure is the only freedom left.

\paragraph{The closure has two encodings.} A boundary condition delivers the ratio
$\ell_0 = \phi/|\phi'|$ at the surface, but \eqref{eq:q3relations} wants $z_0$, the point where
the cosine itself vanishes. For $\phi = \cos(x/\ell)$ those are not the same number:
\begin{equation}
...
silently. At $c = 2$ it is worth $72\,\%$ in $a/2$.
...
With the exact $z_0$ on top of that, part (d) is part (a) rewritten: same $\ell$, same $z_0$,
same numbers. It has no independent content.

\paragraph{Part 3(e).} Here the closure is manufactured instead of imported. Ask for the
...
incoming current to zero gives $\mu_0 \phi = J = D|\phi'|$, so
\begin{equation}
    \ell_0 = \frac{D}{\mu_0(c)}
\end{equation}
which is Marshak's $2D$ when $\mu_0 = \tfrac12$ and Mark's $\sqrt3 D$ when $\mu_0 = 1/\sqrt3$.
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{lrrrrrrr}
        ...
        \bottomrule
    \end{tabular}
    \caption{\textcolor{blue}{Parts 3(d) and 3(e), in mean free paths. $\ell_0$ and $z_0$ are
    part (e)'s; part (d) uses Case's $z_0$ and is identical to part (a). The manufactured
    closure tracks the imported one to better than $1\,\%$ up to $c = 1.4$ and drifts to
    $-8\,\%$ at $c = 2$.}}
    \label{tab:q3de}
\end{table}

\begin{notebox}[Why there are so many candidates for $z_0$][blue][coltext=blue]
...
away, so it cannot be derived from inside such a theory. It has to be imported, as in (a) and
(d), or manufactured from a half-range closure, as in (b), (c) and (e). The four values in play
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{lrrrr}
        ...
        \bottomrule
    \end{tabular}
    \caption{\textcolor{blue}{Critical half-thickness $a/2$, parts 3(a)--3(e).}}
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{lrrrr}
        ...
        \bottomrule
    \end{tabular}
    \caption{\textcolor{blue}{Critical radius $\Sigma_t R_c$, parts 3(a)--3(e).}}
    \label{tab:q3all_sphere}
\end{table}

The two classical errors act in opposite directions and are visible in both tables: $1/B$
over-estimates the relaxation length, inflating both dimensions, while the constant $z_0$
over-estimates the extrapolation distance and, being subtracted, deflates them. Since $\ell$ is
weighted by $\pi$ in the sphere but only $\pi/2$ in the slab, the radius is over-estimated
throughout, whereas Marshak's $a/2$ crosses the transport value near $c = 1.52$ --- cancellation
rather than accuracy (Figure~\ref{fig:q3_methods}). The $\mu_0$ closure of part (e) tracks (a)
to better than $1\,\%$ up to $c = 1.4$ and falls $8\,\%$ short by $c = 2$.

None of these is a transport calculation, (a) included: all five are the extrapolated-endpoint
relation on an assumed asymptotic flux. \textbf{Assignment 3 Question 5 puts them against a
converged $S_N$ solve}, which is what settles how good they actually are.
}
