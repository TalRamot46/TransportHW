...
rule mapped to $[0,1]$ evaluates it \uline{exactly} rather than approximately as done in \texttt{pn.marshak\_matrix}.
...
\begin{equation}
    ...
    \label{eq:kupdate}
\end{equation}
the constant $c$ cancelling between numerator and denominator. \textcolor{red}{The iterate is then rescaled by $\max_j\phi_{0,j}$, which
...
Method 2 solves \eqref{eq:pnmatrix} in closed form instead, removing the $\Delta x$ error
altogether and leaving only the truncation in $N$. It is the benchmark the gap column of the
results below is measured against; the derivation is in the Appendix.
...
\end{equation}
whose conservative form integrates over the cell $(r,u)\in[r_{i-1/2}, r_{i+1/2}]\times[\mu_{i-1/2}, \mu_{i+1/2}]$ to
\begin{equation}
...
with $V = \Delta x$ in the slab. Substituting the slab row into \eqref{eq:cellflux} returns \eqref{eq:slabsweep} exactly, and the sphere's two rows return \eqref{eq:sphbalance}. \textbf{This logic is implemented in the function \texttt{sn.cell\_flux}}.
\end{notebox}
...
{\color{green!55!black}
\FloatBarrier
\section*{Appendix: Method 2 --- modal elimination, as an $N$-exact benchmark}
