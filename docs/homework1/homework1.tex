...
The asymptotic diffusion solution gives the diffusion coefficient $D_{\text{as}} = (1-c)\nu_0^2$ and the flux
\begin{equation}
    \phi_{\text{as,diff}}(x) = \frac{1}{2(1-c)\nu_0} e^{-|x|/\nu_0}
\end{equation}
which is exactly Case's asymptotic mode: this approximation reproduces $\phi_{\text{as}}$ by construction and errs from the exact flux only by the transient part.
...
\subsection*{The eigenvalue \& form of flux for $c > 1$}
...
\end{equation}
The extrapolation distance comes from the expansion about $c = 1$,
\begin{equation}
    z_0(c) = 0.710446 \left[ \frac{1}{c} + q\,\frac{(1-c)^2}{c} + \mathcal{O}\!\left(\frac{(1-c)^3}{c}\right) \right]
    \label{eq:z0fit}
\end{equation}
with $0.710446$ Hopf's constant.
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{lrrrrr}
        ...
        \bottomrule
    \end{tabular}
    \caption{Critical dimensions in mean free paths, from the approximate $|\nu_0(c)|$ and the printed $z_0$ fit ($q = -0.0199$), sorry for not changing this to $+$. The last column is the departure of $a/2$ from a reference using the transcendental root and Case's Table 23.}
    \label{tab:q3}
\end{table}
...
per cent by $c = 2$ (Figure~\ref{fig:q3_critical}). All of this explanation to justify that everything after this table uses the exact
inputs instead.
\end{notebox}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Marshak sets the incoming partial current to zero, $\phi/4 + D\phi'/2 = 0$, so $z_0 = \ell_0 = 2D = 2/3$ (as seen in the course). Mark sets the incoming angular flux to zero along $\mu = 1/\sqrt{3}$, so $z_0 = \ell_0 = \sqrt{3}D \approx 0.5774$. Neither input carries the true $c$-dependence: $1/B$ is only the $c \to 1^+$ limit of $|\nu_0|$, and both $z_0$ are constants while the transport $z_0(c) \approx 0.7104/c$ halves across the range.
...
by naming \emph{two} inputs: the relaxation length $\ell$ inside $\cos(x/\ell)$ (which is where the $\nu_0(c)$ term comes from), and
...
Writing $z_0 = \ell_0$, as parts (b) and (c) do above, is therefore that limit taken
...
same numbers. \textbf{It has no independent content} (if I didn't get this wrong). 
...
incoming current to zero and using Zimmerman's rules (shown in slides 18-20 in the relevant course slides, more details in Assignment 3 solution) gives $\mu_0 \phi = J = D|\phi'|$ (at the surface), so
\begin{equation}
    \ell_0 = \frac{D}{\mu_0(c)}
\end{equation}
...
(d) which use Hopf's solution to the Milne problem, or \underline{manufactured} from a half-range closure and \eqref{eq:l0z0}, as in (b), (c) and (e).
\end{notebox}
...
\begin{equation}
    ...
    \label{eq:q4_fv}
\end{equation}
with $A_i = 4\pi r_i^2$ and $V_j = \frac{4\pi}{3}(r_{j+1/2}^3 - r_{j-1/2}^3)$. The matrix is tridiagonal and solved in $O(N)$. \textbf{The appendix~\ref{app:fv} builds the mesh, the index map and the assembly of the tridiagonal matrix in full}.
...
\begin{figure}[htbp]
    ...
    \caption{Multiplication factor against sphere radius for the two fissile materials, both approximations. The circles mark the $k = 1$ crossing. \textbf{Sorry for the captions over the graphs, it's unprofessional but I didn't get to erasing them.}}
    \label{fig:q5}
\end{figure}
