...
\begin{table}[htbp]
    ...
    \begin{tabular}{ccrrr}
        \toprule
        $c$ & $D_0(c)$ & $t$ & classical & asymptotic \\
        \midrule
        0.6 & 0.48588 & 1  & $-19.96\%$ & $-33.70\%$ \\
            &         & 4  & $+11.05\%$ & $-8.02\%$ \\
            &         & 15 & $+23.88\%$ & $+2.61\%$ \\
        \midrule
        0.8 & 0.39629 & 1  & $-24.18\%$ & $-30.47\%$ \\
            &         & 4  & $-0.27\%$  & $-8.54\%$ \\
            &         & 15 & $+8.39\%$  & $-0.59\%$ \\
        \midrule
        1.0 & 0.33333 & 1  & $-27.86\%$ & $-27.86\%$ \\
            &         & 4  & $-8.78\%$  & $-8.78\%$ \\
            &         & 15 & $-2.46\%$  & $-2.46\%$ \\
        \midrule
        1.2 & 0.28717 & 1  & $-31.08\%$ & $-25.75\%$ \\
            &         & 4  & $-15.47\%$ & $-8.93\%$ \\
            &         & 15 & $-10.59\%$ & $-3.67\%$ \\
        \midrule
        1.5 & 0.23745 & 1  & $-35.25\%$ & $-23.29\%$ \\
            &         & 4  & $-23.24\%$ & $-9.05\%$ \\
            &         & 15 & $-19.70\%$ & $-4.85\%$ \\
        \bottomrule
    \end{tabular}
    \caption{Signed relative error at $x = 0$, both approximations, from
    \texttt{check\_diffusion\_error} in \texttt{src/homework2/main.py}.}
    \label{tab:q3d}
\end{table}
\paragraph{Discussing the results.} Table~\ref{tab:q3d} is $E$ signed, at $x = 0$.
We will discuss two observations about the result, each is visible in Figure~\ref{fig:q3d}.

\textbf{Diffusion is a late-time limit, and the table is the rate.} Every column shrinks with
$t$: the exact solution relaxes onto its diffusive shape only once a particle has scattered
many times, and at $t = 1$ --- one mean free time, one collision --- there is nothing diffusive
about it, which is why all ten entries at $t=1$ sit between $-20\%$ and $-35\%$ regardless of
which $D$ is used.

\textbf{Once relaxed, $D_0(c)$ is the better coefficient, and by a large factor.} At $t = 15$
...
where the asymptotic value is the worse of the two ($-33.70\%$ against $-19.96\%$ at $c=0.6$):
