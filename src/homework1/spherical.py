...
def _banded_operator(medium, areas, volumes, h):
...
def neutron_balance(R, medium, n_cells=400):
    ...
    leakage = areas[-1] * medium.D * result.phi[-1] / (0.5 * h)
...
def critical_radius(medium, n_cells=400, xtol=1e-12):
    """Radius at which k = 1, by brentq on k(R) - 1, bracketed around the analytic one."""
    def residual(R):
        return k_eigenvalue(R, medium, n_cells).k - 1.0
...
def mesh_convergence(medium, cell_counts=(25, 50, 100, 200, 400, 800)):
    """(cells, radii, relative errors) against the analytic radius; second order."""
