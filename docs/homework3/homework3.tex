...
\begin{notebox}[Derivation of the sweeping formulas][blue]
\emph{Forward sweep}, $\mu_m>0$: the flux enters at $x_{i-1/2}$ and leaves at $x_{i+1/2}$, so
the closure is used as $\psi_{i+1/2,m}=2\psi_{i,m}-\psi_{i-1/2,m}$. Substituting it into
\eqref{eq:slabbalance} removes the outgoing face, leaving
$2\mu_m\left(\psi_{i,m}-\psi_{i-1/2,m}\right)+\Sigma_t\Delta x\,\psi_{i,m}=\tfrac12 S_i\Delta x$.
Collecting $\psi_{i,m}$ gives the cell centre, and the closure again gives the flux handed to
cell $i+1$:
\begin{equation}
    ...
    \label{eq:slabsweep}
\end{equation}
marched from $x=0$ outwards, starting from the reflected flux there.

\emph{Backward sweep}, $\mu_m<0$: the roles swap. Writing $\mu_m=-|\mu_m|$ turns the leading
term of \eqref{eq:slabbalance} into $|\mu_m|(\psi_{i-1/2,m}-\psi_{i+1/2,m})$, so the inflow is
now at $x_{i+1/2}$ and the closure is used the other way round,
...
\end{equation}
marched inwards from the vacuum face at $x=a/2$, where $\psi_{I+1/2,m}=0$. The two are one
formula with the inflow face exchanged, which is why a single $|\mu_m|$ serves both.
\end{notebox}
...
\end{equation}
whose \texttt{conservative} form (not discussed here) integrates over the cell $(r,\mu)\in[r_{i-1/2}, r_{i+1/2}]\times[\mu_{m-1/2}, \mu_{m+1/2}]$ to
\begin{equation}
...
\begin{notebox}[Sweeping mechanism for spherical shells][blue]
Equation \eqref{eq:sphbalance} is \eqref{eq:slabbalance} with a second outgoing face. The two
are not alike, and it is worth naming them apart:
\begin{itemize}
    \item the \emph{radial face}, weighted by the shell areas $A_{i\pm1/2}$, whose outflow
    feeds the \textbf{next radial cell} at the same ordinate. Which area is the outflow depends
    on the sign of $\mu_m$, exactly as in the slab;
    \item the \emph{angular face}, weighted by the coefficients $\alpha_{m\pm1/2}$ of the
    angular bin, whose outflow feeds the \textbf{next ordinate} in the same cell. This one
    never swaps: $d\mu/ds\ge0$, so the inflow is always at $m-\tfrac12$.
\end{itemize}
Each closes with its own diamond relation,
\begin{align}
    \psi_{i+\frac12,m} &= 2\psi_{i,m} - \psi_{i-\frac12,m}
        \quad (\mu_m>0), \qquad
...
Take the forward sweep, $\mu_m>0$, so the radial outflow is $A_{\text{out}}=A_{i+1/2}$ and the
inflow $A_{\text{in}}=A_{i-1/2}$. Substituting \eqref{eq:closr} and \eqref{eq:closmu} into
\eqref{eq:sphbalance} eliminates both outgoing fluxes at once,
\begin{multline*}
    ...
    + \Sigma_t V_i \psi_{i,m} = \frac{S_i}{2}V_i
\end{multline*}
leaving every term carrying either $\psi_{i,m}$ or a known inflow, so collecting $\psi_{i,m}$
gives the cell solve outright:
\begin{equation}
    \boxed{\;
    ...
    \label{eq:cellflux}
\end{equation}
and \eqref{eq:closr} and \eqref{eq:closmu} then hand on the two fluxes the march needs next:
$\psi_{i+1/2,m}$ to radial cell $i+1$, and $\psi_{i,m+1/2}$ to ordinate $m+1$. The backward
sweep is \eqref{eq:cellflux} with the two areas exchanged, $\mu_m$ replaced by $|\mu_m|$ and
the radial inflow taken as $\psi_{i+1/2,m}$; the angular half is untouched. Setting
$A_{\text{out}}=A_{\text{in}}=1$, $V_i=\Delta x$ and $\alpha_{m\pm1/2}=0$ returns
\eqref{eq:slabsweep} exactly, which is the check that the two cell solves are the same balance.
\textbf{In code they are kept apart on purpose: \texttt{sn.slab.cell\_flux} takes one inflow,
\texttt{sn.sphere.cell\_flux} takes a \texttt{radial\_face} and an \texttt{angular\_face}.}
\end{notebox}
...
\begin{sloppypar}
Every name in Algorithms~\ref{alg:sweep} and~\ref{alg:snk} is the identifier
...
\begin{notebox}[Three things in the table that follow from the argument above][blue]
\begin{enumerate}
    \item \textbf{The sign of the gap is the sign of $l_0$.} Marshak's extrapolation $2/3$ is
    longer than Mark's $1/\sqrt3 = 0.577$, so the $P_N$ flux is driven to zero further outside
    the slab and less material is needed --- $P_N$ is below $S_{N+1}$ at every one of the
    twelve entries.

    \item \textbf{The gap is a boundary effect, so it shrinks with $N$}, from $5$--$9\%$ at
    $N=1$ to under $1\%$ at $N=9$; by then both boundary conditions are resolving the same
    half-range flux and the choice between them stops mattering.

    \item \textbf{It grows with $c$} at fixed $N$ --- $5.13\%$, $7.81\%$, $9.33\%$ at $N=1$ ---
    because a larger $c$ means a thinner slab, a shorter relaxation length and a more
    forward-peaked surface flux, which is exactly the shape a half-range condition of low order
    cannot capture.
\end{enumerate}
\end{notebox}
...
\begin{tabular}{ccccc}
\toprule
material & order & $R_c$ [cm] & change [cm] & $M_c$ [kg] \\
\midrule
Pu-239 & $S_2$    & 4.92418 &           & 7.8522 \\
Pu-239 & $S_4$    & 5.10842 & $+0.18423$ & 8.7669 \\
Pu-239 & $S_8$    & 5.15863 & $+0.05021$ & 9.0280 \\
Pu-239 & $S_{16}$ & 5.17298 & $+0.01435$ & 9.1035 \\
Pu-239 & $S_{32}$ & 5.17693 & $+0.00395$ & 9.1244 \\
Pu-239 & $S_{64}$ & 5.17797 & $+0.00103$ & 9.1299 \\
\midrule
U-235 & $S_2$    & 7.12179 &           & 28.7481 \\
U-235 & $S_4$    & 7.35140 & $+0.22961$ & 31.6193 \\
U-235 & $S_8$    & 7.40721 & $+0.05581$ & 32.3449 \\
U-235 & $S_{16}$ & 7.42305 & $+0.01585$ & 32.5529 \\
U-235 & $S_{32}$ & 7.42744 & $+0.00439$ & 32.6107 \\
U-235 & $S_{64}$ & 7.42860 & $+0.00115$ & 32.6259 \\
\bottomrule
\end{tabular}
\caption{The $S_N$ ladder. The change column falls by roughly a factor of four at every doubling of $N$ --- the measured ratios are $3.7$, $3.5$, $3.6$, $3.8$ --- which is the second-order convergence in $1/N$ the quadrature should have. The last rung moves the radius by $1\times10^{-3}$ cm, and fitting $A/N^2$ to the last two increments puts the error remaining at $S_{64}$ near $3.4\times10^{-4}$ cm for Pu-239 and $3.8\times10^{-4}$ cm for U-235 --- well inside the four decimals the comparison below prints.}
\end{table}
...
\begin{tabular}{cccccc}
\toprule
material & $c$ & method & $R_c$ [cm] & $M_c$ [kg] & vs converged $S_N$ \\
\midrule
Pu-239 & 1.500 & converged $S_{64}$   & 5.1780 & 9.130  & --- \\
Pu-239 & 1.500 & classical diffusion  & 5.8163 & 12.940 & $+12.33\%$ \\
Pu-239 & 1.500 & asymptotic diffusion & 5.1890 & 9.188  & $+0.21\%$ \\
Pu-239 & 1.500 & asymptotic + Milne   & 5.1780 & 9.130  & $+0.0006\%$ \\
\midrule
U-235 & 1.300 & converged $S_{64}$   & 7.4286 & 32.626 & --- \\
U-235 & 1.300 & classical diffusion  & 8.1031 & 42.345 & $+9.08\%$ \\
U-235 & 1.300 & asymptotic diffusion & 7.4339 & 32.696 & $+0.07\%$ \\
U-235 & 1.300 & asymptotic + Milne   & 7.4289 & 32.630 & $+0.0037\%$ \\
\bottomrule
\end{tabular}
\caption{The four methods on the same two spheres. Classical diffusion is out by 9--12\% in radius, which the cube turns into 30--42\% in mass; asymptotic diffusion recovers all but a fifth of a percent; and the asymptotic + Milne relation is inside $4\times10^{-5}$ of the converged transport answer.}
\end{table}


\FloatBarrier
\section*{Every closed form against converged $S_N$}

Assignment 1 Question 3 produced five closed forms for the critical size, all of them the same
extrapolated-endpoint relation $\tfrac{a}{2} = \tfrac{\pi}{2}\ell - z_0$, or
$\Sigma_t R_c = \pi\ell - z_0$ in the sphere, under different choices
of the relaxation length $\ell$ and the extrapolation distance $z_0$. None is a transport
calculation. Questions 3 and 4 above provide the transport answer they were missing, so this is
the place to settle what each is worth.

\begin{table}[htbp]
\centering
\begin{tabular}{lrrrr}
\toprule
 & \multicolumn{2}{c}{slab, $a/2$ [mfp]} & \multicolumn{2}{c}{sphere, $\Sigma_t R_c$ [mfp]} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
method & value & vs $S_N$ & value & vs $S_N$ \\
\midrule
\multicolumn{5}{l}{\emph{$c = 1.2$}} \\
(a) = (d) exact transport & 1.2898 & $+0.03\%$ & 3.1720 & $+0.00\%$ \\
(b) classical, Marshak & 1.3612 & $+5.57\%$ & 3.3891 & $+6.85\%$ \\
(c) classical, Mark & 1.4505 & $+12.49\%$ & 3.4784 & $+9.66\%$ \\
(e) asymptotic, $\mu_0$ closure & 1.3017 & $+0.95\%$ & 3.1839 & $+0.38\%$ \\
converged $S_{64}$ & 1.2894 & --- & 3.1719 & --- \\
\midrule
\multicolumn{5}{l}{\emph{$c = 1.5$}} \\
(a) = (d) exact transport & 0.6076 & $+0.41\%$ & 1.6901 & $+0.00\%$ \\
(b) classical, Marshak & 0.6159 & $+1.78\%$ & 1.8984 & $+12.33\%$ \\
(c) classical, Mark & 0.7052 & $+16.54\%$ & 1.9877 & $+17.61\%$ \\
(e) asymptotic, $\mu_0$ closure & 0.5955 & $-1.59\%$ & 1.6780 & $-0.71\%$ \\
converged $S_{64}$ & 0.6051 & --- & 1.6901 & --- \\
\midrule
\multicolumn{5}{l}{\emph{$c = 1.8$}} \\
(a) = (d) exact transport & 0.3931 & $+1.10\%$ & 1.1830 & $-0.02\%$ \\
(b) classical, Marshak & 0.3473 & $-10.69\%$ & 1.3612 & $+15.05\%$ \\
(c) classical, Mark & 0.4366 & $+12.27\%$ & 1.4505 & $+22.60\%$ \\
(e) asymptotic, $\mu_0$ closure & 0.3699 & $-4.87\%$ & 1.1598 & $-1.98\%$ \\
converged $S_{64}$ & 0.3889 & --- & 1.1832 & --- \\
\bottomrule
\end{tabular}
\caption{Assignment 1's parts 3(a)--3(e) against the $S_N$ solver of Questions 3 and 4, run to
$S_{64}$ on 800 cells, the order Table~11 shows to be converged. Both geometries, same materials, same $c$.}
\end{table}
\end{document}
