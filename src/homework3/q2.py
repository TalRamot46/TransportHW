...
from homework3.pn import box, modal
...
def _table(scan):
    ...
    for c in plots.C_VALUES:
        ...
        for N in ORDERS:
            box_size = scan.sizes[c][N]
            modal_size = modal.critical_half_thickness(medium, N)
            rows.append([f'{c}', f'P{N}', f'{box_size:.6f}', f'{modal_size:.6f}',
                         f'{abs(box_size / modal_size - 1.0):.1e}', f'{reference:.5f}',
                         f'{modal_size / reference - 1.0:+.2%}'])
...
def report(figs):
    ...
    log_section('Assignment 3 Question 2',
                ...
                f'Method 1, box scheme on {box.N_CELLS} cells with the k power '
                'iteration; Method 2, modal elimination and det H(a/2) = 0.',
                'Reflective at x = 0, vacuum at x = a/2.')

    scan = plots.scan_orders(box.critical_half_thickness, box.k_eigenvalue,
