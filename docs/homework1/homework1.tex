...
\paragraph{The outer surface.} Face $N$ has no cell beyond it. The surface flux $\phi_s$ is eliminated between the boundary condition $\phi + \ell_0 \phi' = 0$ and the one-sided difference over the half cell $h/2$ that separates $\bar r_{N-1}$ from the surface:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:CN}
\end{equation}
which is the single line assigning \texttt{conductance[-1]}, the code writing $\ell_0$ as the variable \texttt{l0} (a lower-case L, not a one). Because $\ell_0$ appears only in the denominator, $\ell_0 = 0$ is not a special case: it gives $\phi_s = 0$ and $C_N = D A_N/(h/2)$, the zero-flux limit. The two treatments of \texttt{\_outer\_boundary} are then just two choices of $(R_{\text{out}}, \ell_0)$: the extrapolated zero uses $(R + z_0,\, 0)$, and the Robin condition uses $(R,\, z_0)$.
