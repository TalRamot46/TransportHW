...
\end{equation}
The core has $c_C > 1$, so the coefficient is negative and the solution oscillates; finiteness at the origin selects the sine. The reflector has $c_R \le 1$ and decays, vanishing $z_0(c_R)$ mean free paths past $b$. The relaxation rate is $k_0(c_C)$ per mean free path in the core and $1/\nu_0(c_R)$ in the reflector, so the two profiles are
\begin{equation}
...
The interface at $r = R_C$ carries a flux relation $\phi_R = g\,\phi_C$ and continuity of the net current $J = -D\,d\phi/dr$. In optical variables that current is dimensionless on both sides, $D\,d/dr = D_0\,d/d(\Sigma_t r)$, and the two logarithmic derivatives of $\phi$ at the interface are $k_0\cot(k_0a) - 1/a$ in the core and $-\nu_0^{-1}\coth\!\left(\left[d+z_0\right]/\nu_0\right) - 1/(b-d)$ in the reflector. All three approximations therefore reduce to a single transcendental equation for $a$:
\begin{equation}
    \boxed{\;D_{0,C}\left[k_0\cot\!\left(k_0 a\right) - \frac{1}{a}\right]
    \;+\; g\,D_{0,R}\left[\frac{1}{\nu_0}\coth\!\left(\frac{d + z_0(c_R)}{\nu_0}\right) + \frac{1}{b - d}\right] \;=\; 0\;}
    \label{eq:reflcrit}
\end{equation}
writing $k_0 = k_0(c_C)$ and $\nu_0 = \nu_0(c_R)$. Here $b - d = \Sigma_{t,R}R_C$ is the interface radius counted in \emph{reflector} mean free paths, which is $a$ itself only when the two media share a mean free path; the code keeps the two conversions apart. The reflector enters through nothing but the optical combination $\left[d + z_0(c_R)\right]/\nu_0(c_R)$, its thickness in its own diffusion lengths. The two $1/r$ terms have no slab counterpart: $\phi$ falls as $1/r$ even where $u$ is flat, so a jump in $D$ at a curved interface drives a current by itself. Equation~\eqref{eq:reflcrit} runs from $+\infty$ at $a \to 0$ to $-\infty$ at $a = \pi/k_0$, the unreflected limit, so the fundamental root is always bracketed by $(0,\pi/k_0)$ and \texttt{brentq} finds it in one call.
...
\bottomrule
\end{tabular}
\caption{Asymptotic parameters of the five materials. Sodium is a pure scatterer, $\Sigma_c = \Sigma_f = 0$, so $c_R = 1$ exactly: $\nu_0$ diverges, its inverse is $0$, and the reflector solution degenerates to $u_R$ linear in $r$, with $\nu_0^{-1}\coth\!\left(\left[d+z_0\right]/\nu_0\right) \to \left[d+z_0\right]^{-1}$. The code takes that limit explicitly rather than dividing out a near-zero $\kappa$.}
\end{table}
...
\begin{equation}
    k^{(\ell+1)} = k^{(\ell)}\,
    \textcolor{blue}{\frac{\displaystyle\int_0^{a/2} \frac{c\,\Sigma_t}{k^{(\ell)}}\,\phi_0^{(\ell+1)}\,dx}
    ...
    \label{eq:kupdate}
\end{equation}
\textcolor{blue}{The middle form is what the method actually compares: the fission source
\eqref{eq:box} that $\phi_0^{(\ell+1)}$ would build against the one that produced it, so it is
an exact ratio of sources and not of fluxes that happen to resemble them.} The constant
$c\,\Sigma_t/k^{(\ell)}$ is common to both and cancels. \textcolor{red}{The iterate is then rescaled by $\max_j\phi_{0,j}$, which
...
\begin{sloppypar}
\textcolor{blue}{Every name in Algorithm~\ref{alg:pnbox} is the identifier the code uses.
...
\textcolor{blue}{Equation \eqref{eq:pnmatrix} is a linear system of coupled ODEs with constant
coefficients, so it can also be integrated analytically, with no spatial mesh at all. That
solution is not needed for the results below and is left to the Appendix, where it serves as
the independent check on this one.}
